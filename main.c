...
static void usage(void)
{
 ...
 "Usage: %s [options]\n"
 "Options:\n"
 "  -a <i2c addr>              Specify i2c address of the board to flash.\n"
...
int main(int argc, char * argv [])
{
   ...
   */
  while ((ch = getopt(argc,argv,"?a:b:B:c:C:DeE:Fi:np:OP:qstU:uvVx:yY:")) != -1) {

    switch (ch) {
    case 'a': /* override default programmer baud rate */
      i2c_addr = strtod(optarg, &e);
      if ((e == optarg) || (*e != 0) || i2c_addr == 0) {
        fprintf(stderr, "%s: invalid i2c address specified '%s'\n",
                progname, optarg);
        exit(1);
      }
      break;
  ...
  if (verbose) {
	...
	if ((strcmp(pgm->type, "proger") == 0)) {
		fprintf(stderr, "%sUsing I2C Address              : %d\n", progbuf, i2c_addr);
	}
  }
